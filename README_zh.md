...
assert t1.path == t2.path              # 文件路径相同
assert t1.bandwidths == t2.bandwidths  # 内容完全一致
```
...
| 测试区域 | 验证内容 |
|---|---|
| `list_trace_sets` | 两个 suite 返回预期集合；结果已排序 |
| `list_trace_files` | 返回 Path 列表；按文件名排序；跨调用稳定；test split 正常；all-only 集合使用根目录 |
| `load_trace_file` | 双列解析；时间戳非负；带宽为正；path 字段正确；FCC-16/train 所有文件无报错 |
| 批量加载 | 集合内所有文件加载为等长并行列表 |
| `load_video_sizes` | 6 个码率层均存在；chunk 大小为正；各层 chunk 数一致；高码率层均值更大 |
| 可复现性 | 同 seed 选中相同文件及内容；不同 seed 选中不同文件；`reset(seed=X)` 重置到相同起点 |
